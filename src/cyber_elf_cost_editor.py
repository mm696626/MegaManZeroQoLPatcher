...
def open_cyber_elf_cost_editor(rom_path, game_name):
            ...
            with open(rom_path, 'r+b') as f:
                ...
                for i, var in enumerate(entries):
                    ...
                    if not val_str.isdigit():
                        raise ValueError(f"Invalid input at entry {i + 1}. Must be a non-negative integer.")

                    val = int(val_str)
                    if not (0 <= val <= 65535):
                        raise ValueError(f"Value at entry {i + 1} out of range (0–65535).")
    ...
    for i, var in enumerate(entries):
        tk.Label(editor, text=f"Elf {i + 1}").grid(row=i, column=0, sticky="w", pady=2)
        tk.Entry(editor, textvariable=var, width=8).grid(row=i, column=1, padx=4)

    tk.Button(editor, text="Save and Close", command=write_values).grid(row=num_entries + 1, column=0, columnspan=2, pady=10)
    editor.grab_set()
